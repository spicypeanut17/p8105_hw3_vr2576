...
# The most common snowfall value is 0 cm (2008508 days), which reflects days when no snow was observed. This makes sense as snowfall is a less frequent event compared to other weather conditions. The other most common values were NA (missing data), 25, and 13 mm. 
...
heatmap <- ggplot(ny_noaa_hw3, aes(x = tmin, y = tmax)) +
  ...
  theme_minimal()
heatmap
ggsave("temp_relationship_heatmap.jpg", width = 8, height = 6, dpi = 100)
...
ridge_plot
ggsave("snowfall_dist_ridge.jpg", width = 8, height = 6, dpi = 100)
...
# Three-panel plot 
(heatmap | densityplot / ridge_plot)
...
**What are the most commonly observed snowfall values? Why?**
The most common snowfall value is `snowfall$count` cm (2008508 days), which reflects days when no snow was observed. This makes sense as snowfall is a less frequent event compared to other weather conditions. The other most common values were NA (missing data), 25, and 13 mm. 
...
The ridge plot shows variation between snowfall amounts depending on the time of year, as expected. Most of the snowfall is between 0 to 30 mm. 
...
demographics <- read_csv(here("data", "nhanes_covar.csv"))
accelerometer <- read_csv(here("data", "nhanes_accel.csv"))
...
merged_data <- merged_data |>
  full_join(total_activity, by = "seqn")
...
# Load the CSV files & observe the datasets
jan2020 <- read_csv(here("data/citibike", "Jan 2020 Citi.csv")) |>
  clean_names() |>  
  mutate(year = 2020,
         month = 1)  # Add two columns to track which month and year

jan2024 <- read_csv(here("data/citibike", "Jan 2024 Citi.csv")) |>
  clean_names() |>  
  mutate(year = 2024,
         month = 1)  

july2020 <- read_csv(here("data/citibike", "July 2020 Citi.csv")) |>
  clean_names() |>  
  mutate(year = 2020,
         month = 7)  

july2024 <- read_csv(here("data/citibike", "July 2024 Citi.csv")) |>
  clean_names() |>  
  mutate(year = 2024,
         month = 7) 
